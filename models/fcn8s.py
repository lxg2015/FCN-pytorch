...
class FCN8(BasicModule):
    def __init__(self, path, n_class=21):
        super(FCN8, self).__init__(path) # pascol voc 21 class
        # conv1
        ...
        # fr ?? score from, final layer; upscore ?? upsample score
        self.score_pool3 = nn.Conv2d(256, n_class, 1)
        self.score_pool4 = nn.Conv2d(512, n_class, 1)
        self.score_fr = nn.Conv2d(4096, n_class, 1)

        self.upscore_final2 = nn.ConvTranspose2d(n_class, n_class, 4, 
                                    stride=2, bias=False)
        self.upscore_pool4 = nn.ConvTranspose2d(n_class, n_class, 4,
                                    stride=2, bias=False) 
        self.upscore_pool3 = nn.ConvTranspose2d(n_class, n_class, 16,
                                    stride=8, bias=False)
    ...
    def forward(self, x):
        ...
        h = self.upscore_final2(h) # upsample
        upscore2 = h  # 1/16 (h+38)/16

        h = self.score_pool4(pool4) # (h+198)/16 = (h+38)/16+10
        h = h[:, :, 5:5+upscore2.size(2), 5:5+upscore2.size(3)] # (h+38)/16
        score_pool4c = h # 1/16

        h = upscore2 + score_pool4c
        h = self.upscore_pool4(h)
        upscore_pool4 = h  # 1/8 #(h+54)/8

        h = self.score_pool3(pool3) # (h+198)/8=(h+54)/8+18
        h = h[:, :, 9:9+upscore_pool4.size(2),
                    9:9+upscore_pool4.size(3)]
        score_pool3c = h # 1/8 # (h+54)/8

        h = upscore_pool4 + score_pool3c
        h = self.upscore_pool3(h) # 1/1 # (h+62)
    ...
    def copy_params_from_vgg16(self, vgg16):
        ...
        for i, name in zip([0, 3], ['fc6', 'fc7']): # 0,3 is nn.linear
            ...
            l2.weight.data.copy_(l1.weight.data.view(l2.weight.data.size()))
            l2.bias.data.copy_(l1.bias.data.view(l2.bias.data.size()))
